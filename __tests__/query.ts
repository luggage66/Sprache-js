...
describe('Query', () => {
    ...
    const IntegerPlusInteger = Parse.query<{ a: number; b: number }>(
        function*() {
...
describe('QueryOr', () => {
    ...
    it('should ReturnsFirstMatching', () => {
        AssertParser.SucceedsWith(AOrB, 'AAABBC---BB--C--', value =>
            expect(value).toBe('AAA')
        );
        AssertParser.SucceedsWith(AOrB, 'BBAAA----', value =>
            expect(value).toBe('BB')
        );
    });

    it('should fail at starting position?', () => {
        AssertParser.Succeeds(AOrB, 'BBAAA-----', result =>
            expect(result.remainder.position).toBe(2)
        );
        AssertParser.Succeeds(AOrB, 'CAAABB-------', result =>
            expect(result.remainder.position).toBe(1)
        );
